...
function barycentric(p,a,b,c) {
    ...
    var _u=1.0 - _v - _w;
    return { u:_w, v : _v, w : _w };
}

function CanBeSeen(triangle,suportPoint){
    return dot(triangle.n , vec3(subtract(suportPoint , triangle.points[0])))>0;
}
...
function Triangle(a,b,c) {
    ...
    this.Set= function(a,b,c) {    //vec4 (support point)
        ...
        this.points[1] = b;
        this.points[2] = c;
        this.n = normalize(cross(subtract(b, a), subtract(c, a)));
        this.center = multf(0.3, add(add(this.points[0], this.points[1]), this.points[2]));
...
function EPA() {
    ...
    this.AddEdge = function (a, b) {
        for (var z = this.lst_edges.length-1; z >=0 ; --z) {
            if (VecEqual(this.lst_edges[z].points[0], b) && VecEqual(this.lst_edges[z].points[1], a)) {  //interchanged 
    ...
    this.AddNewTriangle = function (a, b, c) {
        if (a[3] < 0 || b[3] < 0 || c[3] < 0) throw "no valid index";
    ...
    this.FindCollision = function (sim) {
        ...
        while (++this.step < _EXIT_ITERATION_LIMIT) {
            ...
            for (var z = 0; z < this.lst_triangles.length; ++z) {
                var dst = Math.abs( dot(this.lst_triangles[z].n, this.lst_triangles[z].points[0]));
            ...
            var dist = dot(entry_cur_support,curTrian.n) - entry_cur_dst;
           ...
           // console.log('curtrian e info', curTrian, dist, '  index=', entry_cur_triangle_it, this.lst_triangles.length, entry_cur_support);
            ...
            if (dist < _EXIT_THRESHOLD) {
                ...
                var vertices = sim.shape1.model.vertices;
                /*
                //local vertices
                var vectl0 = vertices[curTrian.points[0][3]];
                var vectl1 = vertices[curTrian.points[1][3]];
                var vectl2 = vertices[curTrian.points[2][3]];

                //world vertices
                var vect0 = ToWorld(sim.shape1, vectl0);
                var vect1 = ToWorld(sim.shape1, vectl1);
                var vect2 = ToWorld(sim.shape1, vectl2);

                //collision point in world coordinates
                this.colPointW = vec3(add(add(multf(bary.u, vect0),
                                     multf(bary.v, vect1)),
                                     multf(bary.w, vect2)));
                this.colPointL = vec3(add(add(multf(bary.u, vectl0),
                                     multf(bary.v, vectl1)),
                                     multf(bary.w, vectl2)));
*/
                // collision normal
            ...
            DrawPoint(entry_cur_support, vec4(0, 0, 1, 1));
            this.removed = [];
            ...
            for (var z = this.lst_triangles.length-1; z>=0 ; --z) {
                ...
                var see=CanBeSeen(triangle, entry_cur_support);
                if (see) {
