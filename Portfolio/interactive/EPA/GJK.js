...
function supportPoint(p,r) {
    this.p = p;
    this.shapePoint = r;
    if (p.length > 3) throw "corrupted data";
}
...
function getFarthestPointInDirection(shape, dir) {
 ...
 //   throw new Error('This is not an error. This is just to abort javascript');
    return new supportPoint(vec3(ret), returnVector);
}
...
function support(shape1,shape2, dir)
{
  ...
  //  console.log(p1,p2);
    return new supportPoint(subtract(p1.p, p2.p), p1.shapePoint);
}
...
function Simplex() {
    ...
    this.CollisionDetection=function (shape1,shape2)
    {
        ...
        this.dir = negate(this.c.p);//negative direction

        this.b = support(shape1, shape2, this.dir);

        if (dot3(this.b.p, this.dir) < 0)
        {
            return false;
        }
        this.dir = doubleCross(subtract(this.c.p, this.b.p), negate(this.b.p));
    ...
    {
        var ao = vec3(-this.a.p[0], -this.a.p[1], -this.a.p[2]);
        var ab = subtract(this.b.p, this.a.p);
        var ac = subtract(this.c.p, this.a.p);
        var abc = cross(ab, ac);
    ...
    this.tetrahedron=function()
    {
        ...
        var ad = subtract(this.d.p, this.a.p);
