...
function getFarthestPointInDirection(shape, dir) {
    ...
    var maxDot = dot(returnVector, localDir);
    

    for (var z = 0; z < vertices.length ; ++z) {
        var vertex = vertices[z];
        var dt = dot(vertex, localDir);
        if (dt > maxDot) {
            maxDot = dt;
            returnVector =vertex;
    ...
    var ret = vec3(mult(shape.instance.Mat, vec4(returnVector, 1)));
